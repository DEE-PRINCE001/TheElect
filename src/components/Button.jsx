...
const Button = ({text, bg, icon, size, color, shadow, sh, hover, onClick, otherStyles}) => {
  return (
    <button className={`cursor-pointer transition duration-300 ${bg || 'bg-primary'} 
    ${color === 'secondary' ? 'text-primary' : 'text-white'} 
    rounded-lg ${size || 'h-10 w-30 text-xs'} 
